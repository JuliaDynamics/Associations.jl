...
@testset "PredictiveAsymmetry" begin
    ...
    @testset "Binning based" begin
        ...
        @testset "VisitationFrequency" begin
            ...
            # 𝔸(x → y) and  𝔸(x → y | z)
            𝔸reg  = predictive_asymmetry(method, x, y, ηs, normalize = false)
            𝔸cond = predictive_asymmetry(method, x, y, z, method, ηs, normalize = false)
            𝒜reg = predictive_asymmetry(method, x, y, ηs, f = 1.0) # normalize == true by default
            𝒜cond = predictive_asymmetry(method, x, y, z, ηs, f = 1.5) # normalize == true by default
        ...
        @testset "TransferOperator" begin
            ...
            # 𝔸(x → y) and  𝔸(x → y | z)
            𝔸reg  = predictive_asymmetry(method, x, y, ηs, normalize = false)
            𝔸cond = predictive_asymmetry(method, x, y, z, ηs, normalize = false)
            𝒜reg = predictive_asymmetry(method, x, y, ηs, f = 1.0) # normalize == true by default
            𝒜cond = predictive_asymmetry(method, x, y, z, ηs, f = 1.5) # normalize == true by default
