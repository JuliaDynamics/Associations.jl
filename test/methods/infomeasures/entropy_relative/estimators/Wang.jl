...
# `entropy_relative(Shannon(; base), Nx, Ny)` where `Nx` and `Ny`
# are `MvNormal`s.
################################################################
...
re_true = CausalityTools.entropy_relative(Shannon(), Nx, Ny)
re_wang = entropy_relative(Shannon(), WangRE(k = 5, l = 5), Dx, Dy)
re_wang_trans = entropy_relative(Shannon(), WangTransformed(k = 5, l = 5), Dx, Dy)
...
for i = 1:nreps
    Dx = Dataset([rand(Nx) for i = 1:10000])
    Dy = Dataset([rand(Ny) for i = 1:10000])
    re_wang[i] = entropy_relative(Shannon(), WangRE(k = 5, l = 5), Dx, Dy)
    re_wang_trans[i] = entropy_relative(Shannon(), WangTransformed(k = 5, l = 5), Dx, Dy)
end
...
htrue = entropy_relative(Shannon(), Nx, Ny)
