...
ce_x_given_y = entropy_conditional(ConditionalEntropyShannon(), c_xy) |> Rational
@test ce_x_given_y == 11//8

probs_yx = freqs_yx ./ sum(freqs_yx);
c_yx = ContingencyMatrix(probs_yx, freqs_yx);
ce_y_given_x = entropy_conditional(ConditionalEntropyShannon(), c_yx) |> Rational
...
@test entropy_conditional(ConditionalEntropyShannon(), c) >= 0.0
@test entropy_conditional(ConditionalEntropyTsallisAbe(), c) isa Real
@test entropy_conditional(ConditionalEntropyTsallisFuruichi(), c) isa Real
