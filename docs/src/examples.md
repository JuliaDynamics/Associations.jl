...
## [Discretization](@id examples_discretization)
...
# A three-dimensional time series
x = StateSpaceSet(rand(rng, 50, 3))
# Encode each 3-dimensional point using a length-3 ordinal pattern encoding.
d = CodifyPoints(OrdinalPatternEncoding(3))
coded_x = codify(d, x)
```

Multivariate time series can also be encoded by actively considering time information. This is done by treating variable of the time series (each column of a [`StateSpaceSet`](@ref)) separately. For each variable, we apply a sliding window of a certain length, and sequentially encode the elements within the sliding window. Formally, we use [`CodifyVariables`](@ref), which
takes a compatible count-based [`OutcomeSpace`](@ref) as input.

```@example
using Random; rng = Xoshiro(1234)
using ComplexityMeasures
# A three-dimensional time series
d = StateSpaceSet(rand(rng, 50, 3))
# We create a binning scheme by dividing the entire value range of the 
# multivariate time series into five equal-sized bins. 
disc_scheme = CodifyVariables(ValueBinning(5))
# Encode each variable separately according to this binning scheme.
x, y, z = codify(disc_scheme, d) # each variable encoded separately
d_encoded = StateSpaceSet(x, y, z)
```

In contrast to with [`CodifyPoints`](@ref), which yields a 1-dimensional 
encoded time series, [`CodifyVariables`](@ref) preserves the dimension of the original time series.
