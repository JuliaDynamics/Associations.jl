...
function causality(source::Vector{<:Number}, target::Vector{<:Number}, p::VisitationFrequencyTest)
...
function causality(source::T1, target::T2, p::VisitationFrequencyTest) where {
...
function causality(source::T1, target, p::VisitationFrequencyTest) where {
        T1 <: Union{Vector{<:AbstractUncertainValue}, AbstractUncertainValueDataset}, T2}
    causality(resample(source), resample(target), p)
end
