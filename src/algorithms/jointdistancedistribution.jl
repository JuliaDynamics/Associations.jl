...
"""
    joint_distance_distribution(x, y;
        distance_metric::Metric = SqEuclidean(), B::Int = 10, 
        D::Int = 2, τ::Int = 1,
...
function joint_distance_distribution(x, y;
    ...
    for (k, b) in enumerate(1:2*B)
        ...
        # Find the indices of all pairs (i, j) in Dx whose distances fall inside the 
        # interval (bmin, bmax].
    ...
    return HypothesisTests.OneSampleTTest(Δjdd, μ0)
end
