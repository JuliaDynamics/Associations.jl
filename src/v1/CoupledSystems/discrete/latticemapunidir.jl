...
export latticemapunidir

import StaticArrays: SVector
import DynamicalSystemsBase: ContinuousDynamicalSystem

function eom_latticemapunidir(dx, x, p, t)
    f, ε = p[1], p[2]
...
"""
    latticemapunidir(D::Int; uᵢ = rand(D), ϵ::Real = 0.10, f::Function = x -> 2 - x^2) → DiscreteDynamicalSystem
...
"""
function latticemapunidir(D::Int; uᵢ = rand(D), ϵ::Real = 0.10, f::Function = x -> 2 - x^2)
    p = (f, ϵ)
    DiscreteDynamicalSystem(eom_latticemapunidir, uᵢ, p)
end