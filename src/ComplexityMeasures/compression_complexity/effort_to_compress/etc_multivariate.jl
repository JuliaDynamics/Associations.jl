...
function compression_complexity(x::AbstractDataset{D, T}, algorithm::EffortToCompress) where {D, T}
    !isnothing(algorithm.alphabet_size) || throw(ArgumentError("Alphabet size must be specified when input is multivariate (e.g. do `compression_complexity(x, EffortToCompress(alphabet_size = 2)`"))
