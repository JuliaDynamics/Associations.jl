...
Unless otherwise specified, estimation is done in the most naive way possible: compute
probability mass functions separately for each space `X`, `Y` and `XY` using a
[`ProbabilitiesEstimator`](@ref), then plug these
...
function estimate(def::TsallisH3, measure::MITsallis, est::ValueHistogram{RectangularBinning{B}},
    ...
    XY_bins = Dataset([Entropies.encode(rb, xyᵢ) for xyᵢ in XY])
