...
export CopulaParametric

"""
    CopulaParametric <: MutualInformationEstimator
    CopulaParametric(d = Normal())
...
"""
struct CopulaParametric{M <: MutualInformation, D, DB} <: MutualInformationEstimator{M}
    definition::M
    d::D
    debias::DB
end

function CopulaParametric(definition = MIShannon(); d = Normal(), debias = true)
    CopulaParametric(definition, d, debias)
end

function information(est::CopulaParametric{<:MIShannon}, x, y)
    X = StateSpaceSet(x)
    Y = StateSpaceSet(y)
    D = StateSpaceSet(X, Y)
