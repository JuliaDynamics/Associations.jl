...
entropy terms, without any bias correction, using any [`DifferentialInfoEstimator`](@ref)
compatible with multivariate data. See examples
[here](@ref example_mi_DifferentialInfoEstimator). If `measure` is not given, then the
...
Some [`MutualInformation`](@ref) measures can be computed using a [`DifferentialInfoEstimator`](@ref),
...
estimate(est::DifferentialInfoEstimator, x, y) = estimate(MIShannon(), est, x, y)
