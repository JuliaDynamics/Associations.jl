...
# estimator `est`, where `e.base` specifies the logarithm base.

# The first argument, the entropy type `e`, is optional and defaults to `Shannon()`, but
# this may not work for all estimators. To compute [`Renyi`](@ref) or [`Tsallis`](@ref)
# mutual information, you'll need to explicitly give `e` together with a compatible
# estimator.
