...
"""
    entropy_conditional([measure::ConditionalEntropy], est::DifferentialInfoEstimator, x, y)
...
the joint entropy and the marginal entropy of `y`, using
the [`DifferentialInfoEstimator`](@ref) `est`, which must be compatible with multivariate data.
...
function marginal_entropies_ce2h(measure::ConditionalEntropy, est::DifferentialInfoEstimator, x, y)
    e = measure.e.definition
    X = StateSpaceSet(x)
    Y = StateSpaceSet(y)
    XY = StateSpaceSet(X, Y)
