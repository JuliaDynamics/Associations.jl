...
# TODO: make a function that accepts multiple inputs, so we don't need to recompute distances.
"""
    smap(X::Dataset; θ = 1.0, k = 1, 
        training = 1:length(x) ÷ 2 - k, 
        predictees = length(x) ÷ 2 + 1:length(x)
...
"""
function smap(x::AbstractStateSpaceSet; 
        θ = 1.0, k = 1, trainees = 1:length(x) ÷ 2, predictees = length(x) ÷ 2 + 1:length(x)-k, metric = Euclidean(), r = 0)
    Mtrain = x[trainees]
    nₜ = length(trainees)
    d = size(x, 2)
    ...
    for i = 1:nq-k - 1
        ...
        b = W * Matrix(Mtrain[1+k:nₜ, 1:1])
        A = W * [repeat([1], nₜ - k) Matrix(Mtrain[1:nₜ - k, 1:1])]
