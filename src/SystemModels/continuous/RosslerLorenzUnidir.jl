...
A definition (parameter container) for a 6-dimension Rössler-Lorenz system model 
consisting of a 3D Rössler system unidirectionally driving a 3D Lorenz system.
The system is determined not only by its parameters, but its time step and 
initial condition.
